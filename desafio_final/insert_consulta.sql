...
SELECT * FROM employee WHERE Salary IS NULL OR Dno IS NULL;

-- Verificar duplicatas (se aplicável)
SELECT Ssn, COUNT(*) FROM employee GROUP BY Ssn HAVING COUNT(*) > 1;


-- Atualizar salários abaixo do mínimo
UPDATE employee SET Salary = 2500 WHERE Salary < 2500;

-- Preencher Super_ssn nulo com um valor padrão (se fizer sentido)
UPDATE employee SET Super_ssn = '888665555' WHERE Super_ssn IS NULL;

-- Calcular INSS (11% do salário)
SELECT Fname, Lname, Salary, ROUND(Salary * 0.11, 2) AS INSS FROM employee;

CREATE VIEW vw_funcionarios_com_inss AS
SELECT 
    e.Fname + ' ' + e.Lname AS NomeCompleto,
    e.Address,
    e.Salary,
    ROUND(e.Salary * 0.11, 2) AS INSS,
    d.Dname AS Departamento
FROM employee e
JOIN departament d ON e.Dno = d.Dnumber;


-- Verificar se todos os projetos têm departamentos válidos
SELECT * FROM project WHERE Dnum NOT IN (SELECT Dnumber FROM departament);
